...
namespace stdA {
	class BroadcastList {
		public:
			...
			struct InsertNoticeDB
			{
				std::string m_msg;
				uint32_t replay_count_in;
				uint32_t refresh_time_min_in;
				uint32_t target_in;
				SYSTEMTIME reserveDate_in;
				InsertNoticeDB(std::string msg,
					uint32_t replay_count,
					uint32_t refresh_time,
					uint32_t target,
					SYSTEMTIME reserveDate)
				{
					clear();
					m_msg = msg;
					...
					uint32_t target)
				{
					...
					refresh_time_min_in = refresh_time;
					target_in = target;
					GetLocalTime(&reserveDate_in);
				};
				InsertNoticeDB(uint32_t _ul = 0u) {
					clear();
				};
				void clear() {
					memset(this, 0, sizeof(InsertNoticeDB));
				};
			};
