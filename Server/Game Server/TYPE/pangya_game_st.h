...
#define LIMIT_GRAND_PRIX_TICKET		5	// Limit de Grand Prix Ticket que o player pode ter, chegou nesse limit não drop mais ele do hole
...
#define TITLE_GM_TYPEID 964689929
...
#define STDA_C_ITEM_TIME c[3]
...
#if defined(_WIN32)
			memcpy_s(apelido, sizeof(apelido), "Amigo", 7);
#elif defined(__linux__)
			memcpy(apelido, "Amigo", 7);
#endif
		};
		uint32_t uid;
	...
	struct PlayerRoomInfo {
		...
		unsigned char place;			// Tem o valor 0x0A aqui quase sempre das vezes que vi esse pacote, Pode ser o Place(lugar que o player está) tipo Room = 10(hex:0x0A)
		uint32_t guild_uid;
		char guild_mark_img[12];
