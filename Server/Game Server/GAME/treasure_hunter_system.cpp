...
void TreasureHunterSystem::initialize() {
	...
	if (m_thItem.size() <=0)
	{
		_smp::message_pool::getInstance().push(new message("[TreasureHunterSystem::initialize][Log] Treasure Hunter System nao foi carregado com sucesso!", CL_FILE_LOG_AND_CONSOLE));
	}
