...
ChannelInfo* channel::getInfo() {
	...
	m_fake_connetion = ini.readInt("OPTION", "FAKECONNECTION");

	ci = &m_ci;
	//if (m_fake_connetion == 0)
	//{

	//}
	//else
	//{
	//	ci->curr_user = ci->curr_user + m_fake_connetion;//cnt fake
	//}
#if defined(_WIN32)
...
//	if (m_fake_connetion > 0 && !m_state_fake_connection)
//	{
//		std::string nomes[] = { "Tiger(horn)", "Carlito(peace)","Falcon(rock)", "(horn)Sasori", "DK(pen)Insanum", "DK(pen)Felipe","DK(pen)Kaio","Super(star)", "IC(boom)DC", "HS(pen)Nozes", "HS(pen)Silva","(pen)Clip", "(pen)iris", "BigHoerb(cool)", "Gysmo(sun)", "David(rose)", "Locos", "BS(e48)Winner", "BS(e48)Hio", "BS(PP)Pang", "IP(PP)Super" };
//
//		for (int i = 0; i < m_fake_connetion; i++)
//		{
//			std::random_device rd; // Semente para o gerador de números aleatórios
//			std::mt19937 gen(rd()); // Mersenne Twister 19937, um algoritmo de geração de números aleatórios
//			std::uniform_int_distribution<int> dis(0, m_fake_connetion); // Distribuição uniforme entre 0 e 20
//
...
//			_pci->uid = i + 1;
//			_pci->oid = i + 2;
//			_pci->capability.ulCapability = 0;
//			_pci->sala_numero = numeroAleatorio;
//			_pci->level = (unsigned char)i;
//			_pci->capability.ulCapability = 0;
//			_pci->title = 0;
//			_pci->team_point = 1000;
//#if defined(_WIN32)
//			strcpy_s(_pci->nickname, nomes[i].c_str());
//#elif defined(__linux__)
//			strcpy(pri.nickname, "\\1Bot");
//#endif
//
//		
//			v_pci.push_back(*_pci);
//			if (packet_func::pacote046(p, &_session, std::vector< PlayerCanalInfo > {*_pci }, 1))
//				packet_func::channel_broadcast(*this, p, 0);
//		}
//		if (packet_func::pacote046(p, &_session, v_pci, 4))
//			packet_func::channel_broadcast(*this, p, 0);
//
//		m_state_fake_connection = 1;
//		m_ci.curr_user = (short)v_pci.size();//cnt fake
//	}
...
void channel::enterLobbyGrandPrix(player& _session) {
	...
	try {
		...
		auto test = sgs::gs::getInstance().getInfo().rate.countBitGrandPrixEvent();
		p.addUint32(test);
...
std::vector< stPlayerReward > channel::getAllEligibleToGoldenTime() {
	...
	for (auto& p : v_sessions) {
		...
		// Push player
		players.push_back({ p->m_pi.uid, sgs::gs::getInstance().getInfo().curr_user >= 10, pair_ri.first });
	}
...
void channel::requestExitedFromWebGuild(player& _session, packet* _packet) {
	...
	try {
		...
		if (old_pang != _session.m_pi.ui.pang) {
			...
			// UPDATE ON GAME
			packet p;
			packet_func::pacote0C8(p, &_session, &_session.m_pi);
...
void channel::requestExecCCGVisible(player& _session, packet* _packet) {
	...
	try {
		...
		if (visible)
		{
			_session.m_pi.mi.capability.ulCapability = 0;
		}
		updatePlayerInfo(_session);
...
void channel::requestExecCCGNoticePrize(player& _session, packet* _packet) {
	struct TNoticePrize
	{
		uint32_t oid_send;
		uint32_t range;
		...
		uint32_t item_qntd;
		TNoticePrize(uint32_t ul = 0)
		{
			clear();
		};
		void clear() { memset(this, 0, sizeof(TNoticePrize)); };
		std::string toString() {
			return
				"UN=" + std::to_string(oid_send)
				+ ", UN2=" + std::to_string(range)
				+ ", UN3=" + std::to_string(grade)
				+ ", TYPEID=" + std::to_string(item_typeid)
				+ ", QNTY=" + std::to_string(item_qntd);
		};
	};
	TNoticePrize psi{ 0 };
	std::vector<TNoticePrize>* ps = new std::vector<TNoticePrize>();
	REQUEST_BEGIN("ExecCCGNoticePrize");

	try {


		int count = _packet->readUint32();
		for (auto i = 0; i < count; i++)
		{
			_packet->readBuffer(&psi, sizeof(TNoticePrize));
			// Log
			_smp::message_pool::getInstance().push(new message("[channel::requestExecCCGNoticePrize][NOTICE_PRIZE][Log] player[UID=" + std::to_string(_session.m_pi.uid) + "] enviou um Item[TYPEID="
				+ psi.toString(), CL_ONLY_CONSOLE));
		}
	}
	catch (exception& e) {

		_smp::message_pool::getInstance().push(new message("[channel::requestExecCCGNoticePrize][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));
...
void channel::requestOpenTicketReportScroll(player& _session, packet* _packet) {
	...
	try {
		_smp::message_pool::getInstance().push(new message("[channel::requestOpenTicketReportScroll][Log] Player[UID=" + std::to_string(_session.m_pi.uid)
			+ "] Hex: \n\r"
			+ hex_util::BufferToHexString(_packet->getBuffer(), _packet->getSize()), CL_ONLY_CONSOLE));
	...
	try {
		...
		if (qntd > 0) {
			...
			if (coupon.id != 0) {
				...
				else if (wi_coupon->_typeid == 436207674)
				{
					// 20 % de desconto
					cookie = (uint64_t)(cookie * 0.50f);
			...
			try {
				...
				uint32_t qntd_scratchy = (uint32_t)std::floor(cookie / 1000);
				qntd_scratchy = qntd_scratchy + (cookie % 1000 >= 500, 1, 0);
				if (qntd_scratchy > 0) //envia presente ao gastar cookies !
				{
					// Reward
					stItem gift{ 0 };
					...
					gift.type = 2;
					gift.id = -1;
					gift._typeid = 436208243;
					gift.qntd = qntd_scratchy;
					gift.STDA_C_ITEM_QNTD = qntd_scratchy;

					std::string msg = "Comprou algo no shop ? ganhou presente !";

					MailBoxManager::sendMessageWithItem(0, _session.m_pi.uid, msg, gift);
				}
			}
			catch (exception& e) {
			...
			if (pang > 0) {
				packet p;
				packet_func::pacote0C8(p, &_session, &_session.m_pi, pang);
				packet_func::session_send(p, &_session, 1);
			}
...
void channel::requestExtendRental(player& _session, packet* _packet) {
	...
	try {
		...
		// Att pang no Jogo
		packet_func::pacote0C8(p, &_session, &_session.m_pi, part->valor_rental);
...
void channel::requestCharacterStatsUp(player& _session, packet* _packet) {
	...
	try {
		...
		packet_func::pacote0C8(p, &_session, &_session.m_pi, enchant->pang);
		packet_func::session_send(p, &_session, 1);
...
void channel::requestPlayMemorial(player& _session, packet* _packet) {
	...
	try {
		...
		// Memorial System
		auto c = sMemorialSystem::getInstance().findCoin(coin->_typeid, _session.m_pi.uid);
		...
		if (win_item.empty())
		{
			win_item = sMemorialSystem::getInstance().drawCoin(_session, *c);
			if (win_item.empty())
				throw exception("[channel::requestPlayMemorial][Error] player[UID=" + std::to_string(_session.m_pi.uid) + "] tentou jogar Memorial com a coin[TYPEID="
...
void channel::requestUseItemBuff(player& _session, packet* _packet) {
	...
	try {
		...
		if (pIb != nullptr) {	// já tem um equipado aumenta o tempo dele
			...
			ib.end_date = UnixToSystemTime(SystemTimeToUnix(ib.use_date) + (static_cast<long long>(tli->time) * 60/*FROM MINUTES TO SECONDS*/));
...
void channel::requestSendMail(player& _session, packet* _packet) {
	...
	try {
		...
		if (count_item > 0) {
			...
			// Update Pang
			packet_func::pacote0C8(p, &_session, &_session.m_pi, pang_price);
	...
	void channel::requestTakeItemFomMail(player& _session, packet* _packet) {
		...
		try {
			...
			auto ei = _session.m_pi.m_mail_box.getEmailInfo(email_id, true); // Não ler o email(antes era false, já que eu peguei ele)
...
void channel::addSession(player* _session) {
	...
	//else
	//{
	//	m_ci.curr_user = m_ci.curr_user+ m_fake_connetion;//cnt fake
	//}
...
void channel::makePlayerInfo(player& _session) {
	...
	pci.l_unknown = 2;			// Ví players com valores 2 e 4 e 0
...
int channel::CommandByChat(player& _session, std::stringstream command)
{

	std::string cmd = "command_chat";//wind
	room* r = nullptr;
	command >> cmd;
	int code = -1;
	try {
		//comandos via chat ;)
		packet p;
		if (!cmd.empty() && _session.m_pi.mi.capability.stBit.game_master)
		{
			BEGIN_FIND_ROOM(_session.m_pi.mi.sala_numero);
			if (cmd.compare("@notice") == 0)
			{
				std::string notice;
				std::getline(command, notice);
				sgs::gs::getInstance().authCmdBroadcastNotice(std::string("[NOTICIA]: " + notice));
			}
			if (cmd.compare("@autoplay") == 0 && r != nullptr && r->getNumPlayers() > 1)
			{
				// Add Bot Tourney Visual para a sala
				code = 1;
				r->CommandReady();//inicia tudo mundo aqui
				...
				p.addString("[COMMAND_CHAT]");
				p.addString("c0xff00ff00\\cInicializando a room Com Sucesso !");
				packet_func::session_send(p, &_session, 1);
			}
			if (cmd.compare("@wind") == 0 && r != nullptr)
			{
				code = 1;
				uint16_t wind = 0;
				uint16_t degree = 0;
				...
				p.addUint16(wind);	// Wind
				p.addUint16(degree);				// Degree
