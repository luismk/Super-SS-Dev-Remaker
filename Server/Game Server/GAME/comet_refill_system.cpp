...
void CometRefillSystem::initialize() {
	...
	if (m_comet_refill.size() <=0)
	{
		_smp::message_pool::getInstance().push(new message("[CometRefillSystem::initialize][Log] Comet Refill System nao Carregado com sucesso!", CL_FILE_LOG_AND_CONSOLE));
	}
//#ifdef _DEBUG
//#else
