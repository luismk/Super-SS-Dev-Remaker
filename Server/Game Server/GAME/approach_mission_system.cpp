...
void ApproachMissionSystem::initialize() {
	...
	if (m_mad.size() <= 0)
	{
		_smp::message_pool::getInstance().push(new message("[ApproachMissionSystem::initialize][Log] Approach Mission System nao foi carregado com sucesso!", CL_FILE_LOG_AND_CONSOLE));
	}
	// Carregado com sucesso[server::DisconnectSession][ErrorSystem]
	m_load = true;
...
mission_approach_ex ApproachMissionSystem::drawMission(uint32_t _num_players) {
	...
	//preciso criar uma proc aqui pra conntrolar esse valor de missao
	if (((std::rand() % 1000) / 10) <= 50) { // 75% de chance de sair mission
...
//mission_approach_ex ApproachMissionSystem::getMission(uint32_t mission, uint32_t _num_players) {
//
//	mission_approach_ex ma{ 0 };
//
//	if (!isLoad())
...
//	std::srand((uint32_t)std::clock() * 7777 * (uint32_t)(uint64_t)&ma);
//	//preciso criar uma proc aqui pra conntrolar esse valor de missao
//	if (((std::rand() % 1000) / 10) <= 75) { // 75% de chance de sair mission
//
//		auto index = std::rand() % m_mad.size();
//
//		if (m_mad[index].numero == mission) {
//
//			ma.numero = (unsigned char)m_mad[index].numero;
//			ma.box_qntd = (unsigned char)m_mad[index].box;
//			ma.tipo = m_mad[index].tipo;
