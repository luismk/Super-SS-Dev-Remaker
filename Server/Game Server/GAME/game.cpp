...
void Game::SetRoomInfo(RoomLog _info)
{
	m_room_log = _info;
}
...
DropItemRet Game::requestInitDrop(player& _session) {
	...
	if (mana_drop._typeid != 0 && !drop_event_active) // Dropou, false = true, true = false
	{
	...
	if (m_ri.qntd_hole == ci.seq_hole && m_ri.tipo != RoomInfo::TIPO::GRAND_PRIX) {
		...
		if (gp_ticket._typeid != 0 && !drop_event_active) // Dropou, false = true, true = false
...
void Game::requestCalculePang(player& _session) {
	...
	float pang_rate = 0.f;
	auto bonus = pgi->data.bonus_pang;
	...
	pgi->data.bonus_pang = (uint64_t)(pgi->data.bonus_pang * pang_rate);
}
...
void Game::requestReadSyncShotData(player& _session, packet* _packet, ShotSyncData& _ssd) {
	...
	try {
		...
		if (_ssd.pang > 30000u || _ssd.bonus_pang > 9000u)
		{
...
void Game::UpdateRoomLogSql(player& _session, PlayerGameInfo* pgi)
{
	try
	{
		//m_room_log.setPlayerInfo(_session.m_pi.uid, _session.m_pi.ei.char_info->_typeid, pgi->hole, pgi->data.score, pgi->data.exp, pgi->data.pang, pgi->data.bonus_pang, pgi->data.tacada_num, pgi->data.total_tacada_num, pgi->shot_data.special_shot.ulSpecialShot, _session.m_pi.pt.id != 0, pgi->data.giveup, pgi->data.time_out, pgi->enter_after_started, pgi->finish_game, pgi->assist_flag, pgi->trofel);
		//NormalManagerDB::add(0, new CmdUpdateRoomLog(m_room_log, true/*Waiter*/), Game::SQLDBResponse, this);
	}
	catch (exception& e) {

		//_smp::message_pool::getInstance().push(new message("[Game::UpdateRoomLogSql][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));
	}
}
