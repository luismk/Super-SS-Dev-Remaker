...
void item_manager::initItemFromBuyItem(PlayerInfo& _pi, stItem& _item, BuyItem& _bi, bool _shop, int _option, int _gift_opt, int _chk_lvl) {
	...
	case iff::CLUBSET:
	{
		...
		if (_bi.time > 0) {
			...
			{
				_bi.time = _item.qntd;//atualiza para hora atual
				_item.qntd = 1;
			}
			_item.flag = 0x20;
...
item_manager::RetAddItem::TYPE item_manager::addItem(stItem& _item, uint32_t _uid, unsigned char _gift_flag, unsigned char _purchase, bool _dup) {
	...
	try {
		...
		case iff::CADDIE:
		{
			...
			//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Adicionou Caddie");
		...
		case iff::CAD_ITEM:
		{
			...
			if (ci.parts_typeid == _item._typeid) {	// Já tem o parts caddie, atualiza o tempo
				...
				//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Atualizou Caddie Item");
				...
				//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Adicionou Caddie Item");
			}
		...
		case iff::MASCOT:
		{
			...
			if ((pMi.id > 0)/*hasFound*/) {	// Player já tem o mascot, só add mais tempo à ele
				...
				//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Atualizou o tempo[Dia(s)=" + std::to_string(_item.STDA_C_ITEM_TIME) + "] do Mascot");
			}
			else {
				MascotInfoEx mi{};
				mi.id = _item.id;
...
#if defined(_WIN32)
				memcpy_s(mi.message, sizeof(mi.message), "PangYa JP", 15);
#elif defined(__linux__)
				memcpy(mi.message, "PangYa JP", 15);
#endif

				if (mascot->msg.active)
#if defined(_WIN32)
					memcpy_s(mi.message, sizeof(mi.message), "PangYa JP", 16);
#elif defined(__linux__)
					memcpy(mi.message, "PangYa JP", 16);
#endif
				...
				//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Adicionou Mascot");
			}
		...
		case iff::BALL:
		{
			...
			if ((pWi.id > 0)/*hasFound*/) {	// já tem atualiza quantidade
				...
				//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Atualizou Ball");
			}
			else {	// não tem, add
				...
				//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Adicionou Ball");
			}
		...
		case iff::CLUBSET:
		{
			...
			if ((pWi.id > 0)/*hasFound*/) { // Já tem, verifica se é de tempo e atualizar, se não dá error
				...
				if (_item.STDA_C_ITEM_TIME > 0 && ((pWi.flag & 0x20) || (pWi.flag & 0x40) || (pWi.flag & 0x60)) && pWi.end_date_unix_local > 0) {
					...
					//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Atualizou ClubSet");
				...
				//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Adicionou ClubSet");
			}
		...
		case iff::CARD:
		{
			...
			if ((pCi.id > 0)/*hasFound*/) {	// Já tem o item atualiza quantidade
				...
				//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Atualizou Card");
				...
				//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Adicionou Card");
			}
		...
		case iff::FURNITURE:
		{
			...
			//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Adicionou Furniture");
		...
		case iff::AUX_PART:
		{
			...
			if ((pWi.id > 0)/*hasFound*/) {
				...
				//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Atualizou AuxPart");
			}
			else {
				...
				//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Adicionou AuxPart");
			}
		...
		case iff::ITEM:
		{
			...
			if ((pWi.id > 0)/*hasFound*/ && _item._typeid != TICKET_REPORT_SCROLL_TYPEID) {
				...
				//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Atualizou Item");
			}
			else {
				...
				//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Adicionou Item");
			}
		...
		case iff::SKIN:
		{
			...
			//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Adicionou Skin");
		...
		case iff::PART:
		{
			...
			//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Adicionou Part" + (wi.ano > 0 && (wi.ano / 24) == 7/*Dias*/ && wi.flag == 0x60/*Rental*/ ? " Rental[Dia(s)=" + std::to_string(wi.STDA_C_ITEM_TIME) + "]" : std::string("")));
		...
		case iff::HAIR_STYLE:
		{
			...
			if (hair != nullptr) {
				...
				if ((ce.id > 0)/*hasFound*/) {	// Tem o Character
					...
					//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Atualizou HairStyle");
		...
		case iff::MATCH:	// Troféu
		{
			...
			if (type_trofel == 1 || type_trofel == 2) {
				...
				if (tsi.id > 0) {	// Já tem item add quantidade do Troféu especial
					...
					//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Atualizou Trofel Especial");
				}
				else {
					...
					//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Adicionou Trofel Especial");
				}
				...
				if (tsi.id > 0) {	// Já tem item add quantidade do Troféu Grand Prix
					...
					//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Atualizou Trofel Grand Prix");
				}
				else {
					...
					//STATIC_//ADD_ITEM_SUCESS_MSG_LOG("Adicionou Trofel Grand Prix");
				}
			}
		...
		case iff::ITEM:
		{
			...
			if (pWi != nullptr && _item._typeid != TICKET_REPORT_SCROLL_TYPEID) {	// Já tem item add quantidade do Item
				...
				if (sPremiumSystem::getInstance().isPremiumTicket(pWi->_typeid)) {
					...
					auto time = getLocalTimeDiffDESC(st);
					if (time >0 /*|| _session.m_pi.m_cap.stBit.premium_user*/)
					{
