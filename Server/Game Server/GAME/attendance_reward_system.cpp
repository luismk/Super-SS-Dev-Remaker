...
void AttendanceRewardSystem::initialize() {
	...
	if (v_item.size() <= 0)
	{
		_smp::message_pool::getInstance().push(new message("[AttendanceRewardSystem::initialize][Log] Attendance Reward System nao foi Carregado com sucesso.", CL_FILE_LOG_AND_CONSOLE));
	}
...
void AttendanceRewardSystem::sendGrandPrixTicket(player& _session) {
	...
	if (pWi == nullptr || pWi->STDA_C_ITEM_QNTD < 50) {
		...
		item.qntd = (pWi == nullptr) ? 3 : ((50 - pWi->STDA_C_ITEM_QNTD) >= 3 ? 3 : 50 - pWi->STDA_C_ITEM_QNTD);
...
//void AttendanceRewardSystem::sendScratchCardTicket(player& _session) {
//	CHECK_SESSION("sendScratchCardTicket");
//
//	TRY_CHECK;
//
//	auto pWi = _session.m_pi.findWarehouseItemByTypeid(SCRATCH_CARD_TICKET);
//
//	// Envia os 3 Grand Prix para o player, ele n�o tem nenhum ticket ou n�o atingiu o limite
//	if (pWi == nullptr || LIMIT_SCRATCH_CARD_TICKET < pWi->STDA_C_ITEM_QNTD) {
//
//		stItem item{ 0 };
//
//		item.type = 2;
//		item.id = -1;
//		item._typeid = SCRATCH_CARD_TICKET;
//		item.qntd = (pWi == nullptr) ? 3 : ((LIMIT_SCRATCH_CARD_TICKET - pWi->STDA_C_ITEM_QNTD) >= 3 ? 1 : LIMIT_SCRATCH_CARD_TICKET - pWi->STDA_C_ITEM_QNTD);
//		item.STDA_C_ITEM_QNTD = (short)item.qntd;
//		item.STDA_C_ITEM_TIME = 7;		// 7 dias	// no original é no C[3] o tempos
//		// UPDATE ON SERVER AND DB
//		auto rt = item_manager::RetAddItem::T_ERROR;
//
//		if ((rt = item_manager::addItem(item, _session, 0, 0)) < 0/*Error*/)
//			throw exception("[AttendanceRewardSystem::sendScratchCardTicket][Error] player[UID=" + std::to_string(_session.m_pi.uid)
//				+ "] tentou adicionar o Scratch Card Ticket do login, mas nao conseguiu. Bug", STDA_MAKE_ERROR(STDA_ERROR_TYPE::ATTENDANCE_REWARD_SYSTEM, 9, 0));
//
//		
//		// UPDATE ON GAME, s� envia se for diferente de Pang and Exp Pouch
//		if (rt != item_manager::RetAddItem::T_SUCCESS_PANG_AND_EXP_AND_CP_POUCH) {
...
//	LEAVE_CHECK;
//
//	CATCH_CHECK("sendScratchCardTicket");
//	END_CHECK;
//}
...
void AttendanceRewardSystem::requestUpdateCountLogin(player& _session, packet *_packet) {
	...
	std::string msg = "Suas recompensas de Login chegaram!";
	...
	//// D� 3 Grand Prix Ticket, por que � a primeira vez que o player loga no dia
	//sendScratchCardTicket(_session);
