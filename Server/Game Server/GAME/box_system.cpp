...
#ifdef _DEBUG
		// !@ Teste
		_smp::message_pool::getInstance().push(new message("[BoxSystem::initialize][Log] Box System nao foi Carregado com sucesso!", CL_FILE_LOG_AND_CONSOLE));
#endif // _RELEASE
		//#ifdef _DEBUG
	}
...
ctx_box_item* BoxSystem::drawBox(player& _session, ctx_box& _ctx_b) {
	...
	for (auto& el : _ctx_b.item) {
		...
		if (el.active) {
			...
			{
				//_smp::message_pool::getInstance().push(new message("[BoxSystem::drawBox][WARNING] ja tem esse item[TYPEID=" + std::to_string(el._typeid) + "] na conta.", CL_ONLY_CONSOLE));
			}
