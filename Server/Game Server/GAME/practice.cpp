...
void Practice::changeHole(player& _session) {
	...
	INIT_PLAYER_INFO("changeHole", "tentou atualizar o hole do jogo", &_session);
	//UpdateRoomLogSql(_session, pgi);
	...
	if (m_players.size() > 0) {
		...
		for (auto& el : m_players) {
			...
			if (el->m_pi.level < 70/*Ultimo level n�o ganha exp*/)
				pgi->data.exp = 2 * (uint32_t)((hole_seq > 0 ? hole_seq : 0) * TRANSF_SERVER_RATE_VALUE(m_rv.exp));
