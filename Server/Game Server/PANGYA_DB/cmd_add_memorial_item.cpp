...
// Implementa��o da classe CmdAddMemorialItem

#if defined(_WIN32)
#pragma pack(1)
#endif

#include "cmd_add_memorial_item.hpp"
#include "../TYPE/memorial_type.hpp"

using namespace stdA;

CmdAddMemorialItem::CmdAddMemorialItem(bool _waiter) : pangya_db(_waiter) {
}

CmdAddMemorialItem::~CmdAddMemorialItem() {
}


void CmdAddMemorialItem::lineResult(result_set::ctx_res* _result, uint32_t /*_index_result*/) {
//faz nada 
}

response* CmdAddMemorialItem::prepareConsulta(database& _db) {
	...
	for (auto& el : m_coin) {
		...
		for (auto& el2 : el.second.item) 
		{
			auto item = el2;
			r = procedure(_db, m_szConsulta, std::to_string(item.tipo) + ", " +
				std::to_string(item._typeid) + ", " +
				std::to_string(1)
				+ ", " + std::to_string(item.probabilidade) + ", " +
				std::to_string(item.gacha_number) + ", " +
