...
#include "../PANGYA_DB/cmd_1st_anniversary.hpp"
...
#include "../../Projeto IOCP/UTIL/Color.hpp"
...
m_notice(30/*60 segundos 1 minuto para o notice*/), m_ticker(30/*30 segundos para o ticker*/), m_GameGuardAuth(false), m_game_guard(nullptr) {
	...
	try {
		...
		ConsoleEx cmd;
		cmd.ConsoleSetTitle("GameServer: " + std::string(m_si.nome) + " - Players: 0");
...
void game_server::requestLogin(player& _session, packet* _packet) {
	...
	try {
		...
		*(player_info*)pi = cmd_pi.getInfo();
		...
		if (packet_version != m_si.packet_version)
			throw exception("[game_server::requestLogin][Error] Player UID: " + std::to_string(_pi.uid) + ".\tPacket Version of client not match. Server: "
				+ std::to_string(m_si.packet_version) + " == Client: " + std::to_string(packet_version), STDA_MAKE_ERROR(STDA_ERROR_TYPE::GAME_SERVER, 1055, 0));
		...
		NormalManagerDB::add(5, new CmdRegisterLogon(pi->uid, 0/*Logou*/, mac_address), game_server::SQLDBResponse, this);
		...
		NormalManagerDB::add(0, new CmdFirstAnniversary(), game_server::SQLDBResponse, this);

		_smp::message_pool::getInstance().push(new message("[game_server::requestLogin][Player]: ID=" + std::string(pi->id) + ", UID=" + std::to_string(pi->uid) + ", ING="
			+ std::string(pi->nickname) + " Acabou de logar.", CL_FILE_LOG_AND_CONSOLE));
...
void game_server::requestChat(player& _session, packet* _packet) {
	...
	try {
		...
		if (c != nullptr) {
			...
			c->CommandByChat(_session, std::stringstream(msg));
			}
			else {
				packet p;
...
#if defined(_WIN32)
				memcpy_s(cmu.macro[i], sizeof(cmu.macro[i]), "PangYa GameRaze! Por favor configure seu chat macro", 51);
#elif defined(__linux__)
				memcpy(cmu.macro[i], "PangYa GameRaze! Por favor configure seu chat macro", 51);
#endif
...
void game_server::requestCommonCmdGM(player& _session, packet* _packet) {
	...
	try {
		...
		//fazer uma chamada aqui no comando mesmo
		CmdMemberInfo cmd_mi(_session.m_pi.mi.uid, true);	// Waiter
		...
		_session.m_pi.mi.capability = cmd_mi.getInfo().capability;
		...
		case CCG_GIVE_ITEM:
		{
			...
			bi.qntd = item_qntd;
			bi.time = (base->shop.flag_shop.time_shop.active ? base->shop.flag_shop.time_shop.dia : 0);
			...
			auto msg = std::string("GM enviou um item para voce: [ " + std::string(base->name) + " ]");
		...
		case CCG_NOTICE_PRIZE: {

			// Envia item para todos da sala
			if (!(_session.m_pi.m_cap.stBit.game_master/* & 4*/))
				throw exception("[game_server::requestCommonCmdGM][NOTICE_PRIZE][Error] player[UID=" + std::to_string(_session.m_pi.uid) + "] nao eh GM mas tentou executar comando GM. Hacker ou Bug", STDA_MAKE_ERROR(STDA_ERROR_TYPE::GAME_SERVER, 1, 0x5700100));

			if (_session.m_pi.m_cap.stBit.block_give_item_gm)
				throw exception("[game_server::requestCommonCmdGM][NOTICE_PRIZE][Error] player[UID=" + std::to_string(_session.m_pi.uid) + "] esta bloqueado para enviar itens pelo comando goldenbell.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::GAME_SERVER, 9, 0x5700100));

			auto c = findChannel(_session.m_pi.channel);

			if (c == nullptr)
				throw exception("[game_server::requestCommonCmdGM][NOTICE_PRIZE][Error] player[UID=" + std::to_string(_session.m_pi.uid)
					+ "] tentou executar o comando /noticeprize mas ele nao esta em nenhum canal. Hacker ou Bug", STDA_MAKE_ERROR(STDA_ERROR_TYPE::GAME_SERVER, 8, 0x5700100));

			//c->requestExecCCGNoticePrize(_session, _packet);

			break;
		}
		case CCG_HIO_HOLE_CUP_SCALE: {

			//// Envia item para todos da sala
			//if (!(_session.m_pi.m_cap.stBit.game_master/* & 4*/))
			//	throw exception("[game_server::requestCommonCmdGM][HIO_HOLECUP_SIZE][Error] player[UID=" + std::to_string(_session.m_pi.uid) + "] nao eh GM mas tentou executar comando GM. Hacker ou Bug", STDA_MAKE_ERROR(STDA_ERROR_TYPE::GAME_SERVER, 1, 0x5700100));

			//if (_session.m_pi.m_cap.stBit.block_give_item_gm)
			//	throw exception("[game_server::requestCommonCmdGM][HIO_HOLECUP_SIZE][Error] player[UID=" + std::to_string(_session.m_pi.uid) + "] esta bloqueado para enviar itens pelo comando goldenbell.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::GAME_SERVER, 9, 0x5700100));

			//auto c = findChannel(_session.m_pi.channel);

			//if (c == nullptr)
			//	throw exception("[game_server::requestCommonCmdGM][HIO_HOLECUP_SIZE][Error] player[UID=" + std::to_string(_session.m_pi.uid)
			//		+ "] tentou executar o comando /goldenbell mas ele nao esta em nenhum canal. Hacker ou Bug", STDA_MAKE_ERROR(STDA_ERROR_TYPE::GAME_SERVER, 8, 0x5700100));

			//c->requestExecCCGHioHoleCupPrize(_session, _packet);
			//
			break;
		}
		case CCG_SET_MISSION: {
			...
			//c->requestExecCCGSetMission(_session, _packet);
			break;
		}
		case CCG_MATCH_MAP:
		default:
...
void game_server::requestSendTicker(player& _session, packet* _packet) {
	...
	try {
		...
		try {
			...
			_session.m_pi.consomeCookie(1000);
...
#error Unknown pointer size or missing size macros!
	...
	try {
		...
		_session->requestSendBuffer(mb.buf, mb.len);//por algum motivo esta travando o envio
	}
	catch (exception& e) {
...
void game_server::onHeartBeat() {
	...
	try {
		...
		cmd.ConsoleSetTitle("GameServer: " + std::string(m_si.nome) + " - Players: " + std::to_string(m_si.curr_user));
		...
		/*ConsoleEx a;
		a.ConsoleSetTitle("Game Server: " + std::string(m_si.nome) + " - Players: " + std::to_string(m_si.curr_user));*/
	}
	catch (exception& e) {
...
bool game_server::checkCommand(std::stringstream& _command) {
	...
	if (!s.empty() && s.compare("exit") == 0)
		return true;	// Sai
	if (!s.empty() && s.compare("clear") == 0)
	{
#ifdef _WIN32
		system("cls");
		// Código específico para limpar a tela do console no Linux e macOS
#else
	...
	else if (!s.empty() && s.compare("broadcast") == 0)
	{
		std::string notice;
		std::getline(_command, notice);
		authCmdBroadcastNotice(std::string("[NOTICIA]: " + notice));
		_smp::message_pool::getInstance().push(new message("[game_server::authCmdBroadcastNotice][Log]: " + notice, CL_ONLY_CONSOLE));
	}
	else
...
bool game_server::checkPacket(session& _session, packet* _packet) {
	...
	if (_session.m_check_packet.checkPacketId(_packet->getTipo())) {
		...
		uint32_t limit_count = CHK_PCKT_COUNT_LIMIT + 5;	// Padrão
...
void game_server::config_init() {
	...
	m_si.rate.exp = m_reader_ini.readInt("SERVERINFO", "EXPRATE");
	m_si.rate.scratchy = m_reader_ini.readInt("SERVERINFO", "SCRATCHY_RATE");
	m_si.rate.pang = m_reader_ini.readInt("SERVERINFO", "PANGRATE");
	m_si.rate.club_mastery = m_reader_ini.readInt("SERVERINFO", "CLUBMASTERYRATE");
	m_si.rate.papel_shop_rare_item = m_reader_ini.readInt("SERVERINFO", "PAPEL_RATE_RATE"); ;
	m_si.rate.papel_shop_cookie_item = m_reader_ini.readInt("SERVERINFO", "PAPEL_COOKIE_ITEM_RATE"); ;
	m_si.rate.treasure = m_reader_ini.readInt("SERVERINFO", "TREASURE_RATE"); ;
	m_si.rate.memorial_shop = m_reader_ini.readInt("SERVERINFO", "MEMORIAL_RATE");
	m_si.rate.chuva = m_reader_ini.readInt("SERVERINFO", "CHUVA_RATE");
	m_si.rate.grand_zodiac_event_time = (m_reader_ini.readInt("SERVERINFO", "GZ_EVENT") >= 1 ? 1 : 0);// Ativo por padrão
	m_si.rate.grand_prix_event = (m_reader_ini.readInt("SERVERINFO", "GP_EVENT") >= 1 ? 1 : 0);// Ativo por padrão
	m_si.rate.golden_time_event = (m_reader_ini.readInt("SERVERINFO", "GOLDEN_TIME_EVENT") >= 1 ? 1 : 0);// Ativo por padrão
	m_si.rate.login_reward_event = (m_reader_ini.readInt("SERVERINFO", "LOGIN_REWARD") >= 1 ? 1 : 0);// Ativo por padrão
	m_si.rate.bot_gm_event = (m_reader_ini.readInt("SERVERINFO", "BOT_GM_EVENT") >= 1 ? 1 : 0);// Ativo por padrão
	m_si.rate.smart_calculator = (m_reader_ini.readInt("SERVERINFO", "SMART_CALC") >= 1 ? 1 : 0);// Atibo por padrão
	m_si.rate.angel_event = (m_reader_ini.readInt("SERVERINFO", "ANGEL_EVENT") >= 1 ? 1 : 0);// Atibo por padrão
	try {
...
void game_server::init_channelInfo()
{
