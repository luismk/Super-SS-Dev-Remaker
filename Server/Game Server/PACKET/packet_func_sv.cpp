...
#include "../../Projeto IOCP/COMPRESS/compress.h"
...
int packet_func::packet0B7(void* _arg1, void* _arg2) {
	...
	try {
		...
		pri.place = 10;	// 0x0A dec"10" _session.m_pi.place
...
int packet_func::packet0FE(void* _arg1, void* _arg2) {
	...
	try {
		...
		pacote1B1(p, &pd._session, 1);//ver depois
		session_send(p, &pd._session, 0);
...
int packet_func::pacote04A(packet& p, RoomInfoEx& _ri, int option) {
	...
	p.addUint8(_ri.modo);
	if (_ri.modo == RoomInfo::M_REPEAT)//fixar hole
	{
...
int packet_func::pacote0C8(packet& p, player* _session, PlayerInfo* pi, uint64_t opt) {
	if (pi == nullptr)
		throw exception("Erro PlayerInfo *pi is nullptr. packet_func::pacote0C8()", STDA_MAKE_ERROR(STDA_ERROR_TYPE::PACKET_FUNC_SV, 1, 0));
...
//talvez seja a data do servidor
int packet_func::pacote1B1(packet& p, player* _session, int option) {
	std::vector< WarehouseItemEx* > all_ucc;

	for (auto& part : _session->m_pi.mp_wi) {
		if (sIff::getInstance().getItemGroupIdentify(part.second._typeid) == iff::PART && part.second.ucc.idx[0] != '\0') {

			auto ucc_part = sIff::getInstance().findPart(part.second._typeid);

			if (ucc_part != nullptr && (ucc_part->type_item == IFF::Part::UCC_BLANK || ucc_part->type_item == IFF::Part::UCC_COPY))
				all_ucc.push_back(&part.second);
		}
	}
	...
	if (all_ucc.size() > 0u) {
		...
		auto ucc_ctxs = new UCC_Load_Ctx[all_ucc.size()];

		for (uint32_t i = 0u; i < all_ucc.size(); i++) {
			if (all_ucc[i] == nullptr)
				memset(&ucc_ctxs[i], 0, sizeof(UCC_Load_Ctx));
			else {
				ucc_ctxs[i]._typeid = all_ucc[i]->_typeid;
				ucc_ctxs[i].id = all_ucc[i]->id;
				strncpy_s(ucc_ctxs[i].ucc_idx, all_ucc[i]->ucc.idx, 8);
				ucc_ctxs[i].ucc_idx[8] = '\0';
			}
		}
		conversionByte cb((const uint32_t)all_ucc.size() * sizeof(UCC_Load_Ctx), CB_BASE_256);

		unsigned char* tmp = new unsigned char[cb.getNumberIS() + 10];
		size_t compress_out = 0;

		auto _compress = new compress();

		try {

			_compress->compress_data((unsigned char*)ucc_ctxs, all_ucc.size() * sizeof(UCC_Load_Ctx), &tmp[4], &compress_out);
			...
			p.addUint32((uint32_t)GetSystemTimeAsUnix());
			p.addUint32((uint32_t)compress_out + 4u);
			p.addBuffer(tmp, compress_out + 4);
		...
		// !@ esse era o que estava, o padrão
		p.init_plain((unsigned short)0x1B1);
