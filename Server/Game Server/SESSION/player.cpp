		...
		if ((ret = m_pi.addExp(_exp)) >= 0) {
			...
			if (ret > 0) {	// Player Upou de level
				...
				for (auto i = (m_pi.mi.level - ret + 1); i <= m_pi.mi.level; ++i) {
					...
					auto msg = std::string("Voce Evoluiu Meus Parabens! Seu Presente Chegou...");
...
void player::addPang(uint64_t _pang) {
	...
	// UPDATE ON GAME
	packet p;
	packet_func::pacote0C8(p, this, &m_pi, _pang);
	packet_func::session_send(p, this, 1);
};
...
bool player::checkClubSetEquiped(UserEquip& _ue) {
	...
	if (_ue.clubset_id != 0) {
		...
		if (pClubSet == nullptr) {
			...
			auto atual = this->m_pi.findWarehouseItemItById(_ue.clubset_id);
			...
			else if (clubset != NULL && ((pWi.flag & 0x20) || (pWi.flag & 0x40) || (pWi.flag & 0x60)) && pWi.end_date_unix_local > 0)//verifica se é taco de temporario
			{
...
//atualiza e envia pro player
void player::AttMoneys(int opt)
{
	auto old_cookies = this->m_pi.cookie;
	auto old_pangs = this->m_pi.ui.pang;
	// Verifica o Auth Key do player
	CmdCookie cmd_cookie(this->m_pi.uid, true); // Waiter

	cmd_cookie.waitEvent();

	// Verifica o Auth Key do player
	CmdPang cmd_pang(this->m_pi.uid, true); // Waiter

	cmd_pang.waitEvent();

	if (cmd_cookie.getException().getCodeError() != 0)
		throw cmd_cookie.getException();
	...
	if (!opt)
	{
		if (old_cookies != cmd_cookie.getCookie()) {
			packet_func::pacote096(p, m_session, &m_session->m_pi);
			packet_func::session_send(p, m_session, 0);
		}
	}
	else
	{
		if (old_pangs != cmd_pang.getPang()) {
			packet_func::pacote0C8(p, m_session, &m_session->m_pi);
			packet_func::session_send(p, m_session, 0);
		}
	}
}
