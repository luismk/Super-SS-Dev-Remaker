...
namespace stdA {
	...
	public:
		...
		void AttMoneys(int opt = 0);//cookie =0, pang = 1
	...
	public:
		PlayerInfo m_pi;
		GMInfo m_gi;
		uint32_t MAX_SECONDS_CONNECTED = 86400; // Valor máximo de segundos (24 horas)
		// Game Guard
		PlayerGameGuard m_gg;
		//maximo de tempo conectado é 24horas
		uint32_t getSecondsConnected() const {
			// Obter o tempo atual em segundos
			auto currentTime = std::chrono::system_clock::now();
			auto duration = currentTime.time_since_epoch();
			auto seconds = std::chrono::duration_cast<std::chrono::seconds>(duration).count();

			// Calcular a quantidade de segundos conectado (substitua com a lógica real)
			uint32_t secondsConnected = static_cast<uint32_t>(seconds) % MAX_SECONDS_CONNECTED;
