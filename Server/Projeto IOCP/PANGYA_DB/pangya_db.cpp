...
private:
	HANDLE m_hMutex;
	const char url_log[30] = "log\\call_db_cmd.log";
};
	...
	while (1) {
		try {
			...
			if (STDA_SOURCE_ERROR_DECODE(e.getCodeError()) == STDA_ERROR_TYPE::EXEC_QUERY) {
				...
				/*if (true)
			{
					_smp::message_pool::getInstance().push(new message("[Gerou um error grave][Error] Procurar saber: " + std::to_string(GetLastError()), CL_FILE_LOG_AND_CONSOLE));
				test:
					throw;
			}
				goto test;*/
