...
#define MAKE_UNZIP_VECTOR(type, method, iff_name) std::vector< type > v_iff; \
		...
		if (z != nullptr) { \
					...
					if (file != nullptr && ZIP_GET_REAL_SIZE(st.size) >= sizeof(IFF::Head)) { \
						...
						if (head.version == IFF_VERSION/*13*/ && (head.count_element * sizeof(type) + sizeof(head)) == ZIP_GET_REAL_SIZE(st.size)) { \
							...
							if (head.count_element != v_iff.size()) \
								_smp::message_pool::getInstance().push(new message("[IFF::loadVector][log] iff name: " + std::string((iff_name)) + ". Nao carregou todos os elementos do IFF, Tem " + std::to_string(head.count_element) + " element(s) e foi carregado " + std::to_string(v_iff.size()), CL_ONLY_FILE_LOG_TEST)); \
			...
			if (v_iff.empty()) \
				_smp::message_pool::getInstance().push(new message("[E_LOAD_IFF] File " + std::string((iff_name)) + " nao encontrado ou esta vazio.", CL_ONLY_FILE_LOG_TEST)); \
			\
			zip_close(z); \
		}else \
							...
							for (i = 0; i < head.count_element; ++i) { \
								...
								if (_iff.member > 0 || (_iff.member == 0 && map_iff.find(_iff.member) == map_iff.end())) { \
									if (map_iff.find(_iff.member) != map_iff.end()) \
										_smp::message_pool::getInstance().push(new message("[IFF::loadMap][Log] iff name: " + std::string((iff_name)) + " ja tem esse key[typeid=" + std::to_string(_iff.member) + "] no map." , CL_ONLY_FILE_LOG_TEST)); \
									map_iff[_iff.member] = _iff; \
								} \
							} \
							if (head.count_element != map_iff.size()) \
								_smp::message_pool::getInstance().push(new message("[IFF::loadMap][log] iff name: " + std::string((iff_name)) + ". Nao carregou todos os elementos do IFF, Tem " + std::to_string(head.count_element) + " element(s) e foi carregado " + std::to_string(map_iff.size()), CL_ONLY_FILE_LOG_TEST)); \
						}else if (head.version != IFF_VERSION) { \
							zip_fclose(file); \
							zip_close(z); \
							throw exception("Error a versao do arquivo IFF e diferente. Accepted " + std::to_string(IFF_VERSION) + " != " + std::to_string(head.version) + " Read from file. " + std::string((method)), STDA_MAKE_ERROR(STDA_ERROR_TYPE::_IFF, 4, 0)); \
...
bool iff::IsCanOverlapped(uint32_t _typeid) {
	...
	case CLUBSET:
	{
		auto clubset = findClubSet(_typeid);

		if (clubset != nullptr && clubset->shop.flag_shop.time_shop.active)
			return true;
		break;
	}
	case CAD_ITEM:
	...
	case PART:
	{
		...
		// Libera os parts para Duplicatas se ele estiver liberado para vender no personal shop(OLD)
...
IFF::MemorialShopCoinItem& stdA::iff::getMemorialShopCoinItemInfo(uint32_t id)
{
	// TODO: inserir instrução return aqui
	return m_memorial_shop_coin_item.find(id)->second;
};
