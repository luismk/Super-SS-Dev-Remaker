...
void unit_connect_base::translate_packet(session * _session, Buffer * lpBuffer, DWORD dwIOsize, DWORD operation) {
	...
	case STDA_OT_RECV_COMPLETED:
		if (dwIOsize > 0 && lpBuffer != nullptr) 
		{
			...
			//if (ph.size == 0)//verifica se o size é zero, e impede de travar :D
			//{
			//	_smp::message_pool::getInstance().push(new message("pacote vazio, hacker, unit_connect::translate_packet(STDA_OT_RECV_COMPLETED)", CL_FILE_LOG_AND_CONSOLE));
