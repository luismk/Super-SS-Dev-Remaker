		...
		while (__atomic_compare_exchange_n(&m_continue_monitor, &check_m, 1, true, __ATOMIC_RELAXED, __ATOMIC_RELAXED)) {
			...
			try {
				...
				try {
					...
					{
						//posso criar um reboot server aqui
					}
				}catch (exception& e) {
...
#if defined(_WIN32)
				Sleep(5000); // 1 second para próxima verificação
#elif defined(__linux__)
...
#endif
{
	...
	try {	
		...
		inet_pton(AF_INET, m_si.ip, &(addr.sin_addr));
...
void server::accept_completed(SOCKET *_listener, DWORD dwIOsize, myOver *lpBuffer, DWORD _operation) {
	...
	try {
		...
		
		{
			...
			{
				_smp::message_pool::getInstance().push(new message("[server::accept_completed][WARNING] accpet socket controlador nao foi iniciado, nao consegue bloquear conexoes.", CL_ONLY_CONSOLE));
			}
			...
			_smp::message_pool::getInstance().push(new message("[server::acceptEx][Log] Aceitou um player, com IP: " + std::string(_session->getIP()) + ", Key: " + std::to_string((int)_session->m_key) + ", OID: " + std::to_string(_session->m_oid), CL_ONLY_CONSOLE));
			...
			try {
				...
				//if (STDA_SOURCE_ERROR_DECODE(e.getCodeError()) == STDA_ERROR_TYPE::SERVER) {
					...
					if (errCod == 61 || errCod == 62 || errCod == 60)
						m_session_manager.deleteSession(_session);
				//}
		  ...
		  { 
			...
			// Aciona o evento de do accept para colocar mais 1 socket accept no backlog
			SetEvent(EventMoreAccept);
			_smp::message_pool::getInstance().push(new message("[server::accept_completed] Negando player,  by luisMK.", CL_FILE_LOG_AND_CONSOLE));
		}
