// Arquivo mssql.cpp
// Criado em 27/01/2018 as 23:30 Acrisio
// Implentação da classe mssql
...
#define DB_PORT 1433u           // Essa opção só funciona se o sql server for o pago que o express não aceita o por IP

using namespace stdA;

mssql::mssql() : database(DB_HOST, DB_NAME, USER_NAME, USER_PASS, DB_PORT) {
	...
	try {
		...
		_smp::message_pool::getInstance().push(new message("[mssql::mssql][ErrorSystem] " + e.getFullMessageError(), CL_FILE_LOG_AND_CONSOLE));

		m_state = false;
	}
};

mssql::mssql(std::wstring _db_address, std::wstring _db_name, std::wstring _user_name, std::wstring _user_pass, unsigned short _db_port)
...
mssql::mssql(std::string _db_address, std::string _db_name, std::string _user_name, std::string _user_pass, unsigned short _db_port)
	: database(_db_address, _db_name, _user_name, _user_pass, _db_port) {
...
void mssql::connect() {
	...
	try {
		...
		if (is_connected())
			throw exception("[mssql::connect][Error] Ja esta connectado.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::_MSSQL, HAS_CONNECT, 0));
...
void mssql::connect(std::wstring _db_address, std::wstring _db_name, std::wstring _user_name, std::wstring _user_pass, unsigned short _db_port) {
...
void stdA::mssql::connect(std::string _db_address, std::string _db_name, std::string _user_name, std::string _user_pass, unsigned short _db_port) {
	connect(MbToWc(_db_address), MbToWc(_db_name), MbToWc(_user_name), MbToWc(_user_pass), _db_port);
};

void mssql::reconnect() {
    disconnect();
    connect();
};
...
response* mssql::ExecQuery(std::wstring _query) {
   
	if (_query.empty())
        throw exception("[mssql::ExecQuery][Error] _query empty.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::_MSSQL, INVALID_PARAMETER, 0));
...
response* mssql::ExecQuery(std::string _query) {
	std::wstring tmp = MbToWc(_query);

	return ExecQuery(tmp);
};

response* mssql::ExecProc(std::wstring _proc_name, std::wstring _proc_params) {
    
	if (_proc_name.empty())
        throw exception("[mssql::ExecProc][Error] _proc_name empty.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::_MSSQL, INVALID_PARAMETER, 1));
	...
	std::wstring _query = (L"exec " + _proc_name + L" " + _proc_params);
...
response* mssql::ExecProc(std::string _proc_name, std::string _proc_params) {
...
std::string mssql::makeText(std::string _value) {
	return "N'" + _value + "'";
};
...
void mssql::HandleDiagnosticRecord(SQLHANDLE hHandle, SQLSMALLINT hType, ERROR_TYPE _error_type, RETCODE RetCode, std::wstring _query) {
	...
	if (RetCode == SQL_INVALID_HANDLE)
		throw exception(L"Invalid handle!\n", STDA_MAKE_ERROR(STDA_ERROR_TYPE::_MSSQL, INVALID_HANDLE, 0));
	...
	if (RetCode == SQL_SUCCESS_WITH_INFO) {
		...
		throw exception(tmp + (_query.empty() ? L"" : std::wstring(L". Query: " + _query)), STDA_MAKE_ERROR(STDA_ERROR_TYPE::_MSSQL, _error_type, (RetCode != -1 ? RetCode : iError)));
#else
	if (RetCode == SQL_SUCCESS_WITH_INFO) {
		if (!tmp.empty())
